...
@section('content')
    <section class="seccion-reservas seccion-mis-citas">
        <div class="contenedor contenedor-mis-citas">
            ...
            <form class="barra-busqueda-citas barra-busqueda-mis-citas" action="{{ route('cliente.citas.index') }}"
                method="GET">
            ...
            @if ($citas->isEmpty())
                <div class="tarjeta-reserva-vacia tarjeta-vacia-mis-citas">
                    ...
                    <p>{{ $busqueda ? 'Intenta buscar con otro servicio, nombre o teléfono.' : 'Selecciona uno o más servicios, elige tus profesionales y reserva el horario disponible.' }}
                    </p>
                </div>
            @else
                                ...
                                @foreach ($citasGrupo as $cita)
                                    @php
                                        ...
                                        $mensajeNoGestionable = match ($cita->estado) {
                                            'cancelada'
                                                => 'Esta cita ya fue cancelada, por eso no se puede modificar ni eliminar.',
                                            'completada'
                                                => 'Esta cita ya fue cumplida, por eso no se puede modificar ni eliminar.',
                                            'inasistencia'
                                                => 'Esta cita quedó registrada como inasistencia, por eso no se puede modificar ni eliminar.',
                                        ...
                                        $servicios =
                                            $cita->servicios->pluck('nombre_servicio')->join(', ') ?: 'Servicio';
                                    ...
                                    <article
                                        class="tarjeta-cita-registrada tarjeta-cita-limpia tarjeta-cita-con-menu tarjeta-mis-citas">
                                        @if ($puedeGestionarse)
                                            <div class="menu-cita-cliente">
                                                <button type="button" class="boton-menu-cita"
                                                    aria-label="Opciones de cita">•••</button>

                                                <div class="opciones-menu-cita">
                                                    <a href="{{ route('cliente.citas.editar', $cita) }}">Modificar cita</a>

                                                    <form action="{{ route('cliente.citas.cancelar', $cita) }}"
                                                        method="POST" class="form-confirmable">
                                                        @csrf
                                                        @method('DELETE')
                                                        <button type="submit" data-confirm-title="Cancelar cita"
                                        ...
                                        @else
                                            <div class="menu-cita-cliente menu-cita-solo-icono">
                                                <button type="button" class="boton-menu-cita marly-info-cita"
                                                    aria-label="Información de cita" data-info-title="Cita no disponible"
