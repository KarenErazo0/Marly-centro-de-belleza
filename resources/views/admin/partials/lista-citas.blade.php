    ...
    <div class="lista-citas">
        @foreach($citas as $cita)
            ...
            <div class="cita-admin">
                <strong>{{ \Carbon\Carbon::parse($cita->fecha_cita)->translatedFormat('d/m/Y') }} · {{ \Carbon\Carbon::parse($cita->hora_inicio)->format('h:i a') }} - {{ \Carbon\Carbon::parse($cita->hora_fin)->format('h:i a') }}</strong>
                <div class="cita-meta">
                ...
                <p><strong>Servicio:</strong> {{ $cita->servicios->pluck('nombre_servicio')->join(', ') ?: 'Sin servicios registrados' }}</p>
                ...
                @if($puedeMarcarAsistencia)
                    <div class="acciones-asistencia-admin">
                        <form action="{{ route('admin.citas.asistencia', $cita) }}" method="POST" class="form-confirmable">
                            @csrf
                            @method('PATCH')
                            <input type="hidden" name="estado" value="completada">
                            <button type="submit" class="btn-asistencia btn-asistio" data-confirm-title="Confirmar asistencia" data-confirm-message="¿Estas seguro que el cliente sí asistió?" data-confirm-detail="Esta información se reflejará también en el panel del cliente como cita cumplida." data-confirm-action="Sí, asistió"><span>✓</span> Sí asistió</button>
                        </form>
                        <form action="{{ route('admin.citas.asistencia', $cita) }}" method="POST" class="form-confirmable">
                            @csrf
                            @method('PATCH')
                            <input type="hidden" name="estado" value="inasistencia">
                            <button type="submit" class="btn-asistencia btn-no-asistio" data-confirm-title="Confirmar inasistencia" data-confirm-message="¿Estas seguro que el cliente no asistió?" data-confirm-detail="Esta información se reflejará también en el panel del cliente como cita no cumplida." data-confirm-action="Sí, no asistió"><span>×</span> No asistió</button>
                        </form>
                    </div>
                @endif
            </div>
